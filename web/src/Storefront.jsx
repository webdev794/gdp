...
export default function Storefront() {
  ...
  const [phState, setPhState] = useState({ current: 0, prev: null })
  ...
  // A handful of real product names, spread across the catalog rather than
  // just the first few, to seed the rotating search-bar placeholder.
  const searchSuggestions = useMemo(() => {
    if (!products.length) return ['milk', 'bananas', 'bread']
    const names = [...new Set(products.map((p) => p.name.split(/[,(]/)[0].trim().toLowerCase()))]
    const take = Math.min(8, names.length)
    const step = Math.max(1, Math.floor(names.length / take))
    return Array.from({ length: take }, (_, i) => names[i * step])
  }, [products])

  useEffect(() => {
    const id = setInterval(() => {
      setPhState((s) => ({ current: (s.current + 1) % searchSuggestions.length, prev: s.current }))
    }, 2200)
    ...
    <header className="topbar">
      <div className="topbar-row">
        ...
        <label className="searchbar">
          ...
          <span className="searchbar-field">
            <input aria-label="Search groceries" value={query} onChange={(event) => setQuery(event.target.value)} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} placeholder={searchFocused ? 'Search for milk, bananas, bread…' : ''} />
            {!query && !searchFocused && <>
              {phState.prev != null && <span className="searchbar-ph searchbar-ph-out" aria-hidden key={`out-${phState.prev}`} onAnimationEnd={() => setPhState((s) => (s.prev === phState.prev ? { ...s, prev: null } : s))}>Search &ldquo;{searchSuggestions[phState.prev]}&rdquo;</span>}
              <span className="searchbar-ph searchbar-ph-in" aria-hidden key={`in-${phState.current}`}>Search &ldquo;{searchSuggestions[phState.current]}&rdquo;</span>
            </>}
          </span>
        </label>
        <div className="topbar-actions">
          ...
          <button className="cart-pill" type="button" onClick={() => setCartOpen(true)} aria-label={`Cart with ${cartCount} items`}>
            <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true"><path fill="currentColor" d="M7 4h-2l-1 2h2l3.6 7.59-1.35 2.44c-.16.28-.25.61-.25.97 0 1.1.9 2 2 2h12v-2h-11.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.24-5.88a1 1 0 0 0-.88-1.49h-14.2l-.94-2zm14 15c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm-9 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/></svg>
            <span>My Cart</span>
      ...
      {(!searching && !activeCategory) ? (
        ...
        <>
          <nav className="cat-rail" aria-label="Product categories">
            <button className="cat-tile" type="button" onClick={() => { setActiveCategory(null); setQuery('') }}><span className="cat-ico" aria-hidden><svg viewBox="0 0 24 24" width="22" height="22"><path fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M19 12H5m6-7-7 7 7 7"/></svg></span>All</button>
